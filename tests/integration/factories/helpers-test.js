...
module('Integration | Server | Factories | helpers', function(hooks) {
  ...
  test('it creates associations with "association" helper combininig with traits', function(assert) {
    ...
    assert.deepEqual(this.server.schema.posts.find(1).author.attrs, {
      id: '1',
      name: 'Sam'
    });
    assert.deepEqual(this.server.schema.posts.find(1).kind.attrs, {
      id: '1',
      name: 'awesome software'
    });

    assert.equal(this.server.db.authors.length, 1);
    assert.deepEqual(this.server.db.authors[0], { id: '1', name: 'Sam' });
    assert.equal(this.server.schema.authors.find(1).posts.models.length, 1);
    assert.deepEqual(this.server.schema.authors.find(1).posts.models[0].attrs, {
      id: '1',
      title: 'Lorem ipsum',
      authorId: '1',
      kindId: '1'
