...
module('Integration | ORM | hasMany #accessor', function() {
  /*
  ...
  */
  ...
  HasManyHelper.forEachScenario(scenario => {
    test(`the references of a ${scenario.title} are correct`, function(assert) {
      ...
      assert.equal(
        parent[accessor].models.length,
        children.length,
        'parent has correct number of children'
      );
      assert.equal(
        parent[idsAccessor].length,
        children.length,
        'parent has correct number of child ids'
      );

      children.forEach(function(child, i) {
        assert.deepEqual(
          parent[accessor].models[i],
          children[i],
        ...
        if (!child.isNew()) {
          assert.ok(
            parent[idsAccessor].indexOf(child.id) > -1,
