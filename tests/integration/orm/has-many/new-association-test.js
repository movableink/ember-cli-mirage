...
module('Integration | ORM | hasMany #newAssociation', function() {
  HasManyHelper.forEachScenario(scenario => {
    test(`${scenario.title} can build a new associated parent`, function(assert) {
      ...
      assert.deepEqual(
        user[accessor].models[startingCount],
        springfield,
        `the child is appended to the parent's collection`
      );

      if (!user.isNew()) {
        assert.equal(
          springfield[otherIdAccessor],
          user.id,
          `the new address's fk reference the saved parent`
        );
      }
