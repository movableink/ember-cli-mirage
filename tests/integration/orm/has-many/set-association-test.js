...
module('Integration | ORM | hasMany #setAssociation', function(hooks) {
    ...
    if (/^savedParent/.test(scenario.state)) {
      test(`updating an association to a list of mixed children saves the new children and updates all children's fks when ${scenario.title}`, function(assert) {
        ...
        assert.equal(
          savedHomeAddress[otherIdAccessor],
          user.id,
          `the saved child's fk was set`
        );
        assert.equal(
          newHomeAddress[otherIdAccessor],
          user.id,
          `the new child's fk was set`
        );
      });
    }
