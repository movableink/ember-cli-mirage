...
module('Unit | Faker', function() {
  ...
  test('#cycle - cycles the passed data', function(assert) {
    ...
    assert.equal(
      callback(0),
      'first',
      'return the first result for sequence 0'
    );
    assert.equal(
      callback(1),
      'second',
      'return the first result for sequence 1'
    );
    assert.equal(
      callback(2),
      'third',
      'return the first result for sequence 2'
    );
    assert.equal(
      callback(3),
      'first',
      'return the first result for sequence 3'
    );
  });
  ...
  test('#random - returns random element from a list', function(assert) {
    ...
    assert.notEqual(
      ['first', 'second', 'third'].indexOf(callback()),
      -1,
  ...
  test('#range - creates a random number in a range', function(assert) {
    ...
    assert.equal(
      callback() >= min,
      true,
      'result is higher or equal than low value'
    );
    assert.equal(
      callback() <= max,
      true,
      'result is lower or equal than high value'
    );
  });
});
